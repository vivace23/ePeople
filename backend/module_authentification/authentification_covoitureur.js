...
router.get('/profile', authenticateToken, async (req, res) => {
  try {
    const [rows] = await db.query('SELECT nom, prenom, email FROM utilisateurs WHERE id = ?', [req.user.userId]);
    res.json(rows[0]);
  } catch (err) {
...
router.post('/register_covoitureur', async (req, res) => {
    ...
    try {
      ...
      await connection.query(
        'INSERT INTO compte_connexion_covoitureur (username, password_c, id_covoitureur, email) VALUES (?, ?, ?, ?)',
